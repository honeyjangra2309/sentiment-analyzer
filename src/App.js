import logo from './logo.svg';
import './App.css';
import { Navbar,Form,Button } from 'react-bootstrap'

function App() {
  function getData() {
    var inputData=document.getElementById('input').value;
    console.log(inputData);
    var url=`https://twitter-sa-api.herokuapp.com/?query=${inputData}`
    fetch(url).then((Response) => {
        return Response.json();
    }).then((data)=> {
        console.log(data);
        console.log(data.Sentiment.neutral);
        console.log(data.Sentiment.positive);
        console.log(data.Sentiment.negative);
        document.getElementById('result').innerHTML+="neutral: " +data.Sentiment.neutral+"<br />";
        document.getElementById('result').innerHTML+="positive: "+data.Sentiment.positive+"<br />"; 
        document.getElementById('result').innerHTML+="negative: "+data.Sentiment.negative+"<br />"; 
        document.getElementById('result').innerHTML+="<div style='box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px; padding: .5rem; margin: .5rem;margin-bottom: 1rem ;display: block'>"+data.Tweets.tweets[0].Tweet_Text+"<br />";
        document.getElementById('result').innerHTML+="<div style='box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px; padding: .5rem; margin: .5rem; margin-bottom: 1rem ;display: block'>"+data.Tweets.tweets[1].Tweet_Text+"<br />";
        document.getElementById('result').innerHTML+="<div style='box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px; padding: .5rem; margin: .5rem; margin-bottom: 1rem ;display: block'>"+data.Tweets.tweets[2].Tweet_Text+"<br />";
        document.getElementById('result').innerHTML+="<div style='box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px; padding: .5rem; margin: .5rem; margin-bottom: 1rem ;display: block'>"+data.Tweets.tweets[3].Tweet_Text+"<br />";
        document.getElementById('result').innerHTML+="<div style='box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px; padding: .5rem; margin: .5rem; margin-bottom: 1rem ;display: block'>"+data.Tweets.tweets[4].Tweet_Text+"<br />";
     })
    }
  return (
    <div className="App">
      <Navbar bg="dark" variant="dark">
       <Navbar.Brand href="#home">
        <img
          alt=""
          src="data:image/svg+xml;base64,"
          width="30"
          height="30"
          className="d-inline-block align-top"
        />{' '}
        Sentiment Analyzer
      </Navbar.Brand>
    </Navbar>
    <div className="main">
     <Form.Control size="lg" type="text" placeholder="Enter your hashtag" id='input'/>
      <Button variant="dark" type="submit" onClick={getData}>
        Submit
      </Button>
      <div id="result"></div>
    </div>
    
  </div>
  );
}

export default App;
